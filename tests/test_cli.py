...
def test_sweep_muスイープのパラメータが正しく解析される():
    ...
    result = runner.invoke(cli, [
        "sweep", "--u", "20", "--mu-start", "0", "--mu-end", "20", "--mu-step", "2",
        "--nsample", "10", "--dry-run",
    ])
    assert result.exit_code == 0
...
def test_sweep_Uスイープのパラメータが正しく解析される():
    runner = CliRunner()
    result = runner.invoke(cli, [
        "sweep", "--mu", "10", "--u-start", "0", "--u-end", "30", "--u-step", "5",
        "--nsample", "10", "--dry-run",
    ])
    assert result.exit_code == 0
...
def test_sweep_ドライランでシミュレーションは実行されない():
    runner = CliRunner()
    result = runner.invoke(cli, [
        "sweep", "--u", "20", "--mu-start", "0", "--mu-end", "4", "--mu-step", "2",
        "--nsample", "10", "--dry-run",
    ])
    assert result.exit_code == 0
